...
Deno.serve(async (req) => {
  ...
  try {
    const body = (await req.json()) as SyncHrisRequest;
    if (!body.orgId) {
      return json({ error: "orgId is required" }, 400);
    }

    const context = await requireCompanyAccess(req, body.orgId);
    const connection = await getConnectionByCategory(context.supabase, body.orgId, "hris");

    if (!connection?.is_active) {
      return json({ error: "No active HRIS connection found" }, 404);
    }
    ...
    console.error("sync-hris error", error);
