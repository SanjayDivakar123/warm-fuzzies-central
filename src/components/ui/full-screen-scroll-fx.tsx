  ...
  (
    ...
    const changeSection = (to: number) => {
      ...
      leftItemRefs.current.forEach((el, i) => {
        ...
        gsap.to(el, { opacity: i === to ? 1 : 0.4, x: i === to ? 10 : 0, duration: D * 0.6, ease: "power3.out" });
      });
    ...
    const handleLoadedStagger = () => {
      leftItemRefs.current.forEach((el, i) => {
        gsap.fromTo(el, { opacity: 0, y: 20 }, { opacity: i === index ? 1 : 0.4, y: 0, duration: 0.5, delay: i * 0.06, ease: "power3.out" });
      });
      rightItemRefs.current.forEach((el, i) => {
        gsap.fromTo(el, { opacity: 0, y: 20 }, { opacity: i === index ? 1 : 0.4, y: 0, duration: 0.5, delay: 0.2 + i * 0.06, ease: "power3.out" });
      });
    };
    ...
    const css = `
      ...
      .fx-bg-img { position: absolute; inset: -10% 0 -10% 0; width: 100%; height: 120%; object-fit: cover; opacity: 0; will-change: transform, opacity; }
      .fx-bg-overlay { position: absolute; inset: 0; background: var(--fx-overlay); }
      .fx-header { grid-column: 1 / 13; align-self: start; padding-top: 7vh; font-size: clamp(1.5rem, 4vw, 4rem); line-height: 1; text-align: center; color: var(--fx-text); }
      .fx-content { grid-column: 1 / 13; position: absolute; inset: 0; display: grid; grid-template-columns: 1fr 1fr 1fr; align-items: center; height: 100%; padding: 0 var(--fx-grid-px); gap: 2rem; }
      .fx-left, .fx-right { height: 70vh; overflow: hidden; display: grid; align-content: center; }
      .fx-left { justify-items: start; }
      .fx-right { justify-items: end; }
      .fx-track { will-change: transform; width: 100%; }
      .fx-item { color: var(--fx-text); font-weight: 700; letter-spacing: 0em; line-height: 1; margin: calc(var(--fx-row-gap) / 2) 0; opacity: 0.4; position: relative; font-size: 1rem; user-select: none; cursor: pointer; width: 100%; }
      .fx-left-item.active, .fx-right-item.active { opacity: 1; }
      .fx-center { display: grid; place-items: center; text-align: center; height: 70vh; overflow: hidden; }
      .fx-featured { position: absolute; opacity: 0; visibility: hidden; }
      .fx-featured.active { opacity: 1; visibility: visible; }
      .fx-featured-title { margin: 0; color: var(--fx-text); font-weight: 900; letter-spacing: -0.01em; font-size: clamp(3rem, 9vw, 8rem); }
      ...
      .fx-progress { width: 200px; height: 2px; margin: 1rem auto 0; background: rgba(127,127,127,0.2); position: relative; }
      .fx-progress-fill { position: absolute; inset: 0 auto 0 0; width: 0%; background: var(--fx-text); height: 100%; transition: width 0.3s ease; }
      .fx-progress-numbers { position: absolute; inset: auto 0 calc(100% + 6px) 0; display: flex; justify-content: space-between; font-size: 0.75rem; color: rgba(127,127,127,0.5); }
      @media (max-width: 900px) {
        .fx-content { grid-template-columns: 1fr; place-items: center; }
        .fx-left, .fx-right { display: none; }
        .fx-center { height: 80vh; }
      }
    `;
    ...
    return (
      ...
      >
        ...
        {debug && <div style={{ position: "fixed", bottom: 10, right: 10, zIndex: 9999, background: "rgba(255,255,255,0.8)", color: "#000", padding: "6px 8px", font: "12px/1 monospace", borderRadius: 4 }}>Section: {index}</div>}
