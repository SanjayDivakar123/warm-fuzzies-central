      ...
      <DialogContent className="w-[95vw] max-w-4xl max-h-[85vh] overflow-y-auto p-4 sm:p-6">
        ...
        <Card className="border-2">
          <CardContent className="p-3 sm:pt-6 sm:p-6">
            <Badge variant="secondary" className="mb-2 sm:mb-3 text-xs">{currentQuestionData.section}</Badge>
            ...
            <div className="space-y-2">
              {currentQuestionData.options.map((option, index) => (
                <div 
                  key={index}
                  className="p-2 sm:p-3 rounded-lg border bg-muted/30 flex items-start gap-2 sm:gap-3"
                >
                  <span className="font-medium text-muted-foreground min-w-[20px] sm:min-w-[24px] text-sm sm:text-base">
                    {String.fromCharCode(97 + index)})
                  </span>
                  <span className="flex-1 text-sm sm:text-base">{option.text}</span>
                  <Badge 
                    variant="outline" 
                    className={`hidden sm:inline-flex text-xs capitalize flex-shrink-0 ${
        ...
        <div className="border-t pt-3 sm:pt-4 mt-3 sm:mt-4">
          <h4 className="font-medium mb-2 sm:mb-3 text-sm sm:text-base">Section Overview</h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5 sm:gap-2">
            {Object.entries(sections).map(([section, sectionQuestions]) => (
              <button
                key={section}
                onClick={() => setCurrentQuestion(sectionQuestions[0].id - 1)}
                className={`text-left p-2 sm:p-3 rounded-lg border transition-all hover:bg-muted/50 ${
