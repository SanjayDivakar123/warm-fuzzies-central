...
const PremiumAssessment = () => {
  ...
  const premiumQuestions = questionSets[audienceType];
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <Card className="max-w-md w-full">
          <CardContent className="p-6 text-center">
            ...
            <Button onClick={() => navigate('/assessment-selection?type=premium')}>
  ...
  // Check if user has access to premium assessment
  useEffect(() => {
    const checkAccess = () => {
      const paymentKey = `payment_verified_premium_${user?.id}`;
  ...
  useEffect(() => {
    const savedProgress = localStorage.getItem(`premium_assessment_progress_${user?.id}_${audienceType}`);
    if (savedProgress) {
  ...
  useEffect(() => {
    if (user?.id && Object.keys(answers).length > 0) {
      ...
      localStorage.setItem(`premium_assessment_progress_${user?.id}_${audienceType}`, JSON.stringify(progress));
    }
  }, [currentQuestion, answers, user?.id, audienceType]);

  const currentQuestionData = premiumQuestions[currentQuestion];
  const progress = ((currentQuestion + 1) / premiumQuestions.length) * 100;
  ...
  const calculateResults = () => {
    ...
    Object.values(answers).forEach(answer => {
      const selectedOption = premiumQuestions
        .flatMap(q => q.options)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      localStorage.removeItem(`premium_assessment_progress_${user.id}_${audienceType}`);
      
      // Navigate to results with audience type
      navigate(`/premium-results?audience=${audienceType}`);
    } catch (error) {
          ...
          <div className="container-wide py-8 max-w-4xl mx-auto px-4">
            ...
            <div className="text-center mb-8 animate-fade-in">
              ...
              <p className="text-sm text-muted-foreground max-w-2xl mx-auto">
                Discover your leadership color profile with detailed insights and personalized recommendations.
            ...
            <div className="mb-8 animate-fade-in delay-200">
              <div className="flex justify-between items-center mb-3">
                <span className="text-sm font-medium text-muted-foreground">
                  Question {currentQuestion + 1} of {premiumQuestions.length}
                </span>
            ...
            <div className="flex justify-between items-center mt-8 animate-fade-in">
              ...
              >
                {currentQuestion === premiumQuestions.length - 1 ? (
                  isSubmitting ? 'Submitting...' : 'Get Premium Results'
                ) : 'Next'}
